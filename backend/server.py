...
# Serve mobile-frontend for iPhone/iPad (same server as API)
FRONTEND_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'mobile-frontend')
...
def _api_info():
    """JSON payload for API root when frontend is not available."""
    return jsonify({
...
# --- Serve mobile-frontend for iPhone/iPad (open one URL for app + API) ---
@app.route('/')
def serve_app_index():
    """Serve the mobile web app (iPhone/iPad) or API info if frontend not present."""
...
def serve_frontend(path):
    """Serve static files from mobile-frontend (HTML, JS, etc.)."""
    if path.startswith('api/') or path == 'api':
        return jsonify({'error': 'Not found'}), 404
    if not os.path.isdir(FRONTEND_DIR):
        return jsonify({'error': 'Frontend not found'}), 404
    path_clean = path.split('?')[0]
    file_path = os.path.join(FRONTEND_DIR, path_clean)
    if os.path.isfile(file_path):
        return send_from_directory(FRONTEND_DIR, path_clean)
    # Fallback for client-side routes (e.g. /analysis.html)
